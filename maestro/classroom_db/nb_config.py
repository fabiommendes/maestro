...
EXAMPLE = """
from maestro.classroom_db.nb_config import nb_config
...
def nb_config(cfg):
    """
    Replaces nbgrader_config.py.

    Based on "nbgrader quickstart"
    """
    maestro = Classroom.load('config.toml')

    # You only need this if you are running nbgrader on a shared
    # server set up.
    # cfg.Exchange.course_id = maestro.name
    # cfg.Exchange.root = "/tmp/nb-exchange"

    # Update this list with other assignments you want
    cfg.CourseDirectory.db_students = [s.to_json(nbgrader=True) for s in maestro.students]

    # Change the students in this list with that actual students in
    # your course
    cfg.CourseDirectory.db_assignments = [{'name': name} for name in maestro.assignments]
    ...
    cfg.ClearSolutions.text_stub = 'ESCREVA SUA SOLUÇÃO'
