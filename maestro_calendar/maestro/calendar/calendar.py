...
class Calendar:
    ...
    def __init__(self, start, end, data, weekdays=(0, 1, 2, 3, 4), skip=()):
        ...
        line_sizes = (max(map(len, st.splitlines()), default=0) for st in self.data)
        self._width = max(line_sizes, default=40)
    ...
    def _next_date(self, date: datetime.date) -> datetime.date:
        """
...
def parse(src: str) -> Calendar:
    """
    Parse calendar files and return a Calendar object.

    The format is exemplified bellow::
...
def to_rst(src: str) -> str:
    """
    Render calendar from the source markdown format to a well formatted rst table.
    """
    return parse(src).render_rst(str)
